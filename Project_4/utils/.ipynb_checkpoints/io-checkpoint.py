...
def read_hdr_image(image_path: str) -> np.ndarray:
    ...
    hdr_image = imageio.imread(image_path)
    
    # Ensure the image is in the expected format
    if len(hdr_image.shape) == 2:
        # If the image is grayscale (2D), stack to convert to RGB by duplicating the channel
        rgb_hdr_image = np.stack((hdr_image,) * 3, axis=-1)
    elif hdr_image.shape[2] == 3:
        # If the image has 3 channels, assume it is already in RGB format
        rgb_hdr_image = hdr_image  # No need to convert
    elif hdr_image.shape[2] == 4:
        # If there is an alpha channel, discard it by taking only the first three channels (RGB)
        rgb_hdr_image = hdr_image[:, :, :3]
